...
temp_median_year = temps %>% mutate(yr= as.integer(year(ymd(MESS_DATUM_BEGINN))), mo= month(ymd(MESS_DATUM_BEGINN))) %>% filter(mo == 2)%>% group_by(yr,STATIONS_ID)
...
anim = animate(anplot, nframes=15,
